...
def transmit_numpy():
    ...
    # ndarray=(np.dtype(np.float32), (channels,baselines,2)))
...
if sys.argv[-1] == 'tx_numpy':
    cProfile.run('transmit_numpy()') if options.profile else transmit_numpy()
elif sys.argv[-1] == 'tx_std':
    cProfile.run('transmit_std()') if options.profile else transmit_std()
elif sys.argv[-1] == 'rx':
    cProfile.run('receive()') if options.profile else receive()
else:
    print 'Usage: numpy_test.py [options] rx|tx_numpy|tx_std'

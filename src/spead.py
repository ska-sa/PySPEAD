...
def readable_speadpacket(pkt, prepend='', show_payload=False):
    o, rv = 0, ['', 'vvv PACKET ' + 'v'*(50-len(prepend))]
    rv.append('FRAME_CNT=%d, PAYLOAD_LEN=%d, PAYLOAD_CNT=%d' % (pkt.frame_cnt, pkt.payload_len, pkt.payload_cnt))
    for cnt,(is_ext,id,val) in enumerate(pkt.get_items()):
        if is_ext: val = 'EXT OFF=%d LEN=%d' % val
        else: val = '     RAW_VAL=%s' % (hex(val)[2:])
        try: rv.append('ITEM%02d: [ IS_EXT=%d | NAME=%16s | %s ]' % (cnt, is_ext, NAME[id], val))
        except(KeyError): rv.append('ITEM%02d: [ IS_EXT=%d | ID=%18d | %s ]' % (cnt, is_ext, id, val))
    rv.append('PAYLOAD: BYTES=%d' % (pkt.payload_len))
...
def iter_genpackets(frame, use_heap_lenoff=True, max_pkt_size=MAX_PACKET_SIZE):
    ...
    while True:
        ...
        h = ''.join(h)
        logger.debug('itergenpackets: Made packet with hdrlen=%d, heapoff=%d, paylen=%d, heaplen=%d' \
...
def iterframes(tport, max_payloads_in_frame=MAX_PAYLOADS_IN_FRAME):
    ...
    for pkt in tport.iterpackets():
        logger.info('iterframes: Packet with FRAME_CNT=%d, PAYLOAD_CNT=%d' % (pkt.frame_cnt, pkt.payload_cnt))
        logger.debug(readable_speadpacket(pkt, show_payload=False, prepend='iterframes:'))
        # Check if we have finished a frame
        if pkt.frame_cnt != frame_cnt:
            try:
                assert(frame_cnt != -1)
                logger.info('iterframes: Frame %d completed, attempting to unpack heap' % frame_cnt)
                # Throws TypeError if a packet is missing
                heap = ''.join(payload[:max_payload_cnt+1])
                descriptors = frame.pop(DESCRIPTOR_ID)
                for i,(o,length) in enumerate(descriptors): descriptors[i] = heap[o:o+length]
                for id,(is_ext,val) in frame.iteritems():
                    if is_ext:
                        o,length = val
                        # Throws IndexError if data is somehow corrupted:
                        val = heap[o:o+length]
                    else: val = pack(DEFAULT_FMT, val)
                    frame[id] = (is_ext, val)
                frame[DESCRIPTOR_ID] = descriptors
                logger.info('iterframes: Heap successfully unpacked')
                logger.debug(readable_frame(frame, prepend='iterframes:'))
                yield frame
            except(AssertionError,TypeError,IndexError) as ex:
                if type(ex) == AssertionError: pass
                elif type(ex) == TypeError:
                    logger.info('iterframes: Heap unpack failed')
                    for i, pyld in enumerate(payload[:max_payload_cnt+1]):
                        if pyld is None: logger.info('iterframes: Payload for packet_cnt %d was missing' % i)
                elif type(ex) == IndexError:
                    logger.info('iterframes: Heap unpack failed')
                    logger.info('iterframes: ' + str(ex))
        ...
        try:
            ...
            max_payload_cnt = max(max_payload_cnt, pkt.payload_cnt)
            # Throws IndexError if invalid payload cnt/max_payloads_in_frame combo
            payload[pkt.payload_cnt] = pkt.get_payload()
            for (is_ext,id,val) in pkt.get_items():
                if id == DESCRIPTOR_ID: frame[id].append(val)
                else: frame[id] = (is_ext,val)
        except(IndexError):
            logger.warning('iterframes: Invalid payload_cnt in packet, tossing frame')
            frame_cnt = -1
    logger.info('iterframes: Last packet in stream received, processing final frame.')
    try:
        assert(frame_cnt != -1)
        logger.info('iterframes: Frame completed, attempting to unpack heap')
        # Throws TypeError if a packet is missing
        heap = ''.join(payload[:max_payload_cnt+1])
        descriptors = frame.pop(DESCRIPTOR_ID)
        for i,(o,length) in enumerate(descriptors): descriptors[i] = heap[o:o+length]
